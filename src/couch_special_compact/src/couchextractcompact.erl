...
start(#st{} = St, DbName, Options, Parent) ->
    ...
    couch_log:debug("Special Compaction process spawned for db \"~s\"", [DbName]),
    io:format("Special Compaction process spawned for db ~p~n", [DbName]),
...
copy_docs(St, #st{} = NewSt, MixedInfos, Retry) ->
    ...
    NewInfos1 = lists:map(
        fun(Info) ->
            ...
            {NewRevTree, FinalAcc} = couch_key_tree:mapfold(
                fun
                    ({RevPos, RevId}, #leaf{ptr = Sp} = Leaf, leaf, SizesAcc) ->
                        {Body, AttInfos} = copy_doc_attachments(St, Sp, NewSt, DocId),
                        % Since attachments are external, ExternalSize is calculated without them
                        ExternalSize = couch_ejson_size:encoded_size(Body),
                        Doc0 = #doc{
                            id = DocId,
                            revs = {RevPos, [RevId]},
                            deleted = Leaf#leaf.deleted,
                            body = Body,
                            atts = AttInfos  % AttInfos is an empty list
                        },
            ...
            {FinalAS, FinalES, _FinalAtts} = FinalAcc,
            NewActiveSize = FinalAS,     % Attachment sizes are excluded
            NewExternalSize = FinalES,   % Attachment sizes are excluded
            Info#full_doc_info{
                rev_tree = NewRevTree,
                sizes = #size_info{
    ...
    copy_doc_attachments(#st{} = SrcSt, SrcSp, _DstSt, DocId) ->
        ...
        % Remove '_attachments' field from BodyData
        BodyData1 = proplists:delete(<<"_attachments">>, BodyData),
    
        % Add 'externalAttachments' property
        AttachmentNames = [Name || {Name, _Type, _BinSp, _AttLen, _RevPos, _ExpectedMd5} <- BinInfos],
        BodyData2 = [{<<"externalAttachments">>, AttachmentNames} | BodyData1],
    
        % Remove 'handoverBarcode' key if it exists
        BodyData3 = proplists:delete(<<"handoverBarcode">>, BodyData2),
    
        % Return modified BodyData and empty AttInfos
        {BodyData3, []}.
    ...
    read_stream_to_binary(Stream, Acc) ->
        case couch_stream:read(Stream, 4096) of
            {ok, Data} ->
                read_stream_to_binary(Stream, [Acc, Data]);
            eof ->
                BinaryData = iolist_to_binary(Acc),
                {ok, BinaryData};
            {error, Reason} ->
                {error, Reason}
        end.
